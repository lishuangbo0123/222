...
isExcel = False  #是否将数据爬取到excel, 否就把数据存储到数据库
start_excel_line = 2
headers = {
...
def write_data_to(time1,text,type,msg,url):
    ...
    if isExcel:
        ...
        if type == '文章':
            sheet1.cell(start_excel_line, 2, text)
        elif type == '评论':
            sheet1.cell(start_excel_line, 3, text)
        elif type == '子评论':
            sheet1.cell(start_excel_line, 4, text)
        sheet1.cell(start_excel_line, 5, url)
        sheet1.cell(start_excel_line, 6, msg)
        sheet1.cell(start_excel_line, 1, time1)
        excel1.save(uid + '.xls')
        start_excel_line += 1
    else:
        cursor = db.cursor()
...
if __name__ == '__main__':
    ...
    if isExcel:
        ...
        sheet1.cell(1, 1, '时间')
        sheet1.cell(1, 2, '文章内容')
        sheet1.cell(1, 3, '评论内容')
        sheet1.cell(1, 4, '次级评论')
        sheet1.cell(1, 5, 'url')
        sheet1.cell(1, 6, '记录信息')
    else:
